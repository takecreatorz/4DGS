...
class Deformation(nn.Module):
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        ...
        # 追加: 潜在コードを結合
        latent_codes_expanded = latent_codes.unsqueeze(1).expand(-1, hidden.size(1), -1)  # (num_points, hidden_dim, latent_dim)
        hidden = torch.cat([hidden, latent_codes_expanded], dim=-1)
        ##
