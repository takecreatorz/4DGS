...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args, latent_dim: int):
        ...
        # 追加: 潜在コードの初期化
        num_gaussians = self._xyz.shape[0]  # ガウシアンの総数
        latent_dim = args.latent_dim  # 潜在コードの次元数
        self._latent_codes = nn.Parameter(
            torch.randn(num_gaussians, latent_dim, device="cuda") * 0.1,
            requires_grad=True
        )
        self.args = args  # args を属性として保持
        ######
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        # 潜在コードの初期化をここで行う
        num_gaussians = self._xyz.shape[0]  # ガウシアンの総数
        latent_dim = self._latent_codes.shape[1] if hasattr(self, "_latent_codes") else 32  # 潜在コードの次元数
        self._latent_codes = nn.Parameter(
            torch.randn(num_gaussians, latent_dim, device="cuda") * 0.1,
            requires_grad=True
        )
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._latent_codes], 'lr': training_args.latent_lr, "name": "latent_codes"},  # 追加：この行だけ
    ...
    #追加latent_codesを引数に追加
    def compute_deformation(self,time, latent_codes):
        
        deform = self._deformation[:,:,:time].sum(dim=-1)
        # 追加: 潜在コードを空間時間構造エンコーダに渡す
        grid_features = self.grid(self._xyz, latent_codes)
        ###
        # 追加: HexPlaneField から特徴を取得し、変形に加える
        hexplane_features = self.hexplane(self._xyz, time, latent_codes)
        xyz = self._xyz + deform + hexplane_features#hexplane_featuresを足すのを追加
        return xyz
    ...
    def prune_points(self, mask):
        ...
        self._latent_codes = optimizable_tensors["latent_codes"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_latent_codes):
        d = {"xyz": new_xyz,
        ...
        "latent_codes": new_latent_codes  # 追加
        ...
        self._scaling = optimizable_tensors["scaling"]
        self._rotation = optimizable_tensors["rotation"]
        self._latent_codes = optimizable_tensors["latent_codes"]  # 追加
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        if grads.shape[0] != n_init_points:
            print(f"[WARN] mismatch: grads.size={grads.shape}, xyz.size={n_init_points}")
            return
        ...
        new_latent_codes = self._latent_codes[selected_pts_mask].repeat(N, 1)
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_deformation_table, new_latent_codes)

        prune_filter = torch.cat([
            selected_pts_mask,  # 最初の n_init_points に対応(=古いガウシアンたち)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        # 1) まず xyz の個数を取得
        n_init_points = self.get_xyz.shape[0]
        # 2) grads の数が合っているか確認 (デバッグ用)
        if grads.shape[0] != n_init_points:
            print(f"[WARN] mismatch: grads.size={grads.size()}, xyz.size={n_init_points}")
            return  # または例外を投げる
        ...
        selected_pts_mask = torch.logical_and(
            grads_accum_mask,
            torch.max(self.get_scaling, dim=1).values <= self.percent_dense * scene_extent)# 修正
        
        if not selected_pts_mask.any():
            return  # 増やす対象なしなら終了
        #selected_indices = torch.where(grads_accum_mask)[0]  # 条件を満たすインデックスを取得
        #selected_pts_mask[selected_indices] = True  # 選択されたインデックスを設定
        # breakpoint()        
        ...
        ######追加
        #print("selected_pts_mask.shape:", selected_pts_mask.shape)
        #print("self._latent_codes.shape:", self._latent_codes.shape)
        new_latent_codes = self._latent_codes[selected_pts_mask] # 追加
        ######
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_deformation_table, new_latent_codes)#new_latent_codesを追加
    ...
    def densify(self, max_grad, min_opacity, extent, max_screen_size, density_threshold, displacement_scale, model_path=None, iteration=None, stage=None):
        ...
        # ここで再計算してあげる
        new_grads = self.xyz_gradient_accum / self.denom
