...
# train.py の関数追加位置
def latent_code_regularization(latent_codes, lambda_latent):
    """
    Compute the regularization term for latent codes.
    
    Args:
        latent_codes: [num_gaussians, latent_dim] Tensor of latent codes
        lambda_latent: float, regularization weight for latent codes
    
    Returns:
        Regularization loss for latent codes.
    """
    reg_loss = lambda_latent * torch.norm(latent_codes, p=2) ** 2
    return reg_loss
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        # 変更後: 潜在コードの正則化項を損失に追加
        latent_reg_loss = latent_code_regularization(gaussians._latent_codes, opt.lambda_latent)
        loss = Ll1 + latent_reg_loss
        ###
...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname):
    ...
    gaussians = GaussianModel(dataset.sh_degree, hyper, hyper.latent_dim)
...
if __name__ == "__main__":
    ...
    # train.py の parser 定義部分に引数を追加
    #parser.add_argument('--lambda_latent', type=float, default=0.01, help="Regularization weight for latent codes")
