...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        ### ADD HERE: パラメータ差分L2正則化項の追加開始 ###

        # 前フレームのパラメータを保持するための静的変数をこの関数スコープ外で定義できない場合、
        # 関数スコープ内でglobalまたはnonlocal変数を利用、あるいはscene_reconstruction外で定義してください。
        # ここでは簡略のため、scene_reconstruction内で一度だけ定義されると想定します。
        if 'prev_xyz' not in locals():
            prev_xyz = None  # 初回はNone

        current_xyz = gaussians.get_xyz  # 現在フレームのガウス位置パラメータ取得

        if prev_xyz is not None:
            lambda_param = 1e-3  # 正則化項の強度は適宜調整
    
            # prev_xyzとcurrent_xyzの点数数が異なる場合に備えて、最小数の点だけ使う
            minN = min(prev_xyz.shape[0], current_xyz.shape[0])
    
            # 両者とも最初のminN個の点のみを比較
            diff = current_xyz[:minN] - prev_xyz[:minN]
            param_diff_reg = (diff ** 2).mean() * lambda_param
            loss += param_diff_reg

        prev_xyz = current_xyz.detach().clone()

        ### ADD HERE: パラメータ差分L2正則化項の追加終了 ###
