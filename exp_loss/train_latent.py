...
from scene.latent_model import LatentModel  # 新規ファイル(latent_model.py)内に定義した潜在モデルをインポート
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        if stage == "fine" and hyper.time_smoothness_weight != 0:
            ...
            # 追加: 潜在変数間の連続性正則化------------
            latent_vectors = latent_model.get_all_latents()  # ユーザがlatent_modelに定義しておく必要あり
            time_reg = 0
            for i in range(len(latent_vectors)-1):
                time_reg += (latent_vectors[i+1] - latent_vectors[i]).pow(2).sum()
            time_reg = time_reg / (len(latent_vectors)-1)
            loss += hyper.time_smoothness_weight * time_reg
            #-------------------------------------------
            loss += tv_loss
...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname):
    ...
    # 潜在モデルをインスタンス化（latent_dimやlatent_lrはユーザーが定義）
    latent_model = LatentModel(latent_dim=opt.latent_dim).cuda()
    latent_optimizer = torch.optim.Adam(latent_model.parameters(), lr=opt.latent_lr)
    ##
