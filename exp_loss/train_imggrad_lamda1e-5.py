...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        # current frame image: image_tensor (Bx3xHxW)
        # 勾配計算(チャネル毎に同じことするが、RGB平均化や輝度変換してもよい)
        def calc_image_grad(img):
            # img: Bx3xHxWと仮定。バッチ・カラー次元はそのまま処理
            # x方向勾配（差分）
            grad_x = img[:,:,:,1:] - img[:,:,:,:-1]  # Bx3xHx(W-1)
            # y方向勾配（差分）
            grad_y = img[:,:,1:,:] - img[:,:,:-1,:]  # Bx3x(H-1)xW
            return grad_x, grad_y

        if prev_image_tensor is not None:
            lambda_grad = 1e-5  # 正則化強度を調整可能

            grad_x_curr, grad_y_curr = calc_image_grad(image_tensor)
            grad_x_prev, grad_y_prev = calc_image_grad(prev_image_tensor)

            # サイズがずれる可能性があるため、共通部分のみに合わせる
            min_w_x = min(grad_x_curr.shape[-1], grad_x_prev.shape[-1])
            grad_x_diff = (grad_x_curr[:,:,:,:min_w_x] - grad_x_prev[:,:,:,:min_w_x])**2

            min_h_y = min(grad_y_curr.shape[2], grad_y_prev.shape[2])
            grad_y_diff = (grad_y_curr[:,:,:min_h_y,:] - grad_y_prev[:,:,:min_h_y,:])**2
