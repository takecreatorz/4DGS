...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        ### ADD HERE: Acceleration-based Temporal Regularization ###
        if 'prev_xyz' not in locals():
            prev_xyz = None
        if 'prev_prev_xyz' not in locals():
            prev_prev_xyz = None

        current_xyz = gaussians.get_xyz  # 現在フレームのガウス位置 (N,3)程度

        # prev_xyz, prev_prev_xyzが両方存在する場合のみ2次差分を計算
        if prev_xyz is not None and prev_prev_xyz is not None:
            lambda_accel = 1e-4  # 正則化強度は要調整
            minN = min(prev_prev_xyz.shape[0], prev_xyz.shape[0], current_xyz.shape[0])
            v_t = (current_xyz[:minN] - prev_xyz[:minN])
            v_t_1 = (prev_xyz[:minN] - prev_prev_xyz[:minN])
            accel = v_t - v_t_1
            accel_loss = (accel ** 2).mean() * lambda_accel
            loss += accel_loss

        prev_prev_xyz = prev_xyz.detach().clone() if prev_xyz is not None else None
        prev_xyz = current_xyz.detach().clone()
        ### ADD END ###
